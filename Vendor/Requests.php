 ...
 * Requests2 for PHP
 *
 * Inspired by Requests2 for Python.
 *
...
class Requests {
	...
	public static function autoloader($class) {
		// Check that the class starts with "Requests2"
		if (strpos($class, 'Requests2') !== 0) {
			return;
		}
	 ...
	 *    across transports.)
	 *    (string|boolean, default: library/Requests2/Transport/cacert.pem)
	 ...
	 * @param string $type HTTP request type (use Requests2 constants)
	 ...
	 *    {@see Requests2::request}
	 *    (string, required)
	 * - `headers`: Associative array of header fields. Same as the `$headers`
	 *    parameter to {@see Requests2::request}
	 *    (array, default: `array()`)
	 * - `data`: Associative array of data fields or a string. Same as the
	 *    `$data` parameter to {@see Requests2::request}
	 *    (array|string, default: `array()`)
	 * - `type`: HTTP request type (use Requests2 constants). Same as the `$type`
	 *    parameter to {@see Requests2::request}
	 *    (string, default: `Requests2::GET`)
	 * - `cookies`: Associative array of cookie name to value, or cookie jar.
	 *    (array|Requests_Cookie_Jar)
	 *
	 * If the `$options` parameter is specified, individual requests will
	 * inherit options from it. This can be used to use a single hooking system,
	 * or set all the types to `Requests2::POST`, for example.
	 *
	 ...
	 *    (callback)
	 *
	 * @param array $requests Requests2 data (see description for more information)
	 * @param array $options Global and default options (see {@see Requests2::request})
	...
	public static function request_multiple($requests, $options = array()) {
		...
		if (!empty($options['hooks'])) {
			$options['hooks']->register('transport.internal.parse_response', array('Requests2', 'parse_multiple'));
		...
		foreach ($requests as $id => &$request) {
			...
			if ($request['options']['hooks'] !== $options['hooks']) {
				$request['options']['hooks']->register('transport.internal.parse_response', array('Requests2', 'parse_multiple'));
	...
	public static function get_certificate_path() {
		...
		return dirname(__FILE__) . '/Requests2/Transport/cacert.pem';
	}
	 ...
	 * @param array $request Request data as passed into {@see Requests2::request_multiple()}
	 ...
	 *
	 * @codeCoverageIgnore
	 * @deprecated Misspelling of {@see Requests2::flatten}
	 * @param array $array Dictionary of header values
	 * @return array List of headers
	 */
